...
row = float(input('The length of the row, in feet? '))
end = float(input('The amount of space used by an end-post assembly, in feet? '))
space = float(input('The amount of space between the vines, in feet? '))

vine = (row - (2 * end))/space

print('Number of grapevines that will fit in the row:', vine)